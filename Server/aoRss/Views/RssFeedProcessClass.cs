...
namespace Contensive.Addons.Rss.Views {
    ...
    public class RssFeedProcessClass : AddonBaseClass {
        ...
        public override object Execute(CPBaseClass CP) {
            ...
            try {
                ...
                foreach (RSSFeedModel feed in DbBaseModel.createList<RSSFeedModel>(CP, "", "id desc")) {
                    ...
                    Pos = testFilenameNoExt.IndexOf(".xml", StringComparison.OrdinalIgnoreCase);
                    if (Pos >= 0) {
                        testFilenameNoExt = testFilenameNoExt.Substring(0, Pos);
                    }
                    //
                    // -- 20190114, from VS, removes everything before the first Unix slash
                    Pos = testFilenameNoExt.IndexOf("/", StringComparison.OrdinalIgnoreCase);
                    if (Pos >= 0) {
                        testFilenameNoExt = testFilenameNoExt.Substring(Pos + 1);
                    }
                    //
                    ...
                    while (suffixNumber < 100 & ($",{usedFilenames},").IndexOf($",{testFilenameNoExt},", StringComparison.OrdinalIgnoreCase) >= 0) {
                                                ...
                                                if (csStory.OpenRecord(storyContent.name, storyRecordID)) {
                                                    ...
                                                    if (entry.DatePublish == DateTime.MinValue) {
                                                        ...
                                                        if (entry.DatePublish == DateTime.MinValue) {
                                                            ...
                                                            csStory.SetField("dateAdded", entry.DatePublish.ToString());
                                                            if (CP.Content.IsField(storyContent.name, "RSSDatePublish")) {
                                                                csStory.SetField("RSSDatePublish", entry.DatePublish.ToString());
                                                            }
                                                        }
                                                    }
                ...
                foreach (var feed in feedList) {
                    ...
                    //
                    // Build this feed
                    if (!string.IsNullOrEmpty(feed.Link) & feed.Link.IndexOf("://") < 0) {
                        ...
                        if (entry.DateExpires == DateTime.MinValue | entry.DateExpires > RightNow) {
                            if (entry.DatePublish == DateTime.MinValue | entry.DatePublish < RightNow) {
                                ...
                                if (string.IsNullOrEmpty(Link)) {
                                    ...
                                    if (string.IsNullOrEmpty(Link)) {
                                        if (entry.storyContentID == pageContentContentId) {
                                            Link = $"{serverPageDefault}?bid={entry.storyRecordID}".Trim();
                                        }
                                    }
                                }
                                if (Link.IndexOf("://") < 0) {
                                ...
                                if (!string.IsNullOrEmpty(Enclosure)) {
                                    int Ptr = Enclosure.LastIndexOf(".");
                                    string MimeType = "";
                                    if (Ptr >= 0) {
                                        string Ext = Enclosure.Substring(Ptr + 1).ToLowerInvariant();
                                        switch (Ext ?? "") {
                                            case "mp3": {
                                                    MimeType = "audio/mpeg";
                                                    break;
        ...
        private string GetPrimaryDomainName(CPBaseClass cp, string DomainNameList) {
            string result = DomainNameList;
            try {
                if (result.Contains(",")) {
                    string[] CopySplit = cp.Site.DomainPrimary.Split(',');
                    result = CopySplit[0];
                }
                //
            } catch (Exception ex) {
        ...
        private string testEncodeFilename(string Source) {
            ...
            for (int Ptr = 0; Ptr < cnt; Ptr++) {
                string chr = Source.Substring(Ptr, 1);
