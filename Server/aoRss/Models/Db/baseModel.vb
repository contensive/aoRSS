...
Namespace Models.Db
    Public MustInherit Class BaseModel
        ...
        ''' <param name="cp"></param>
        ''' <returns></returns>
        Friend Shared Function add(Of T As BaseModel)(cp As CPBaseClass) As T
        ...
        Friend Shared Function create(Of T As BaseModel)(cp As CPBaseClass, recordId As Integer) As T
            Dim result As T = Nothing
            Try
                If recordId > 0 Then
                    Dim instanceType As Type = GetType(T)
        ...
        Friend Shared Function create(Of T As BaseModel)(cp As CPBaseClass, recordGuid As String) As T
            Dim result As T = Nothing
            Try
                Dim instanceType As Type = GetType(T)
                Dim contentName As String = derivedContentName(instanceType)
        ...
        Friend Shared Function createByName(Of T As BaseModel)(cp As CPBaseClass, recordName As String) As T
            Dim result As T = Nothing
            Try
        ...
        ''' <param name="cp"></param>
        ''' <returns></returns>
        Friend Function save(Of T As BaseModel)(cp As CPBaseClass) As Integer
            Dim cs As CPCSBaseClass = cp.CSNew()
            Try
        ...
        Friend Shared Sub delete(Of T As BaseModel)(cp As CPBaseClass, recordId As Integer)
            Try
                If (recordId > 0) Then
                    Dim instanceType As Type = GetType(T)
                    Dim contentName As String = derivedContentName(instanceType)
        ...
        Friend Shared Sub delete(Of T As BaseModel)(cp As CPBaseClass, ccguid As String)
            Try
        ...
        ''' <returns></returns>
        Friend Shared Function createList(Of T As BaseModel)(cp As CPBaseClass, sqlCriteria As String, sqlOrderBy As String) As List(Of T)
        ...
        ''' <param name="sqlCriteria"></param>
        ''' <returns></returns>
        Friend Shared Function createList(Of T As BaseModel)(cp As CPBaseClass, sqlCriteria As String, sqlOrderBy As String, pageSize As Integer, pageNumber As Integer, Optional listOfLowerCaseFields As List(Of String) = Nothing) As List(Of T)
            Dim result As New List(Of T)
            Try
                Dim cs As CPCSBaseClass = cp.CSNew()
                Dim ignoreCacheNames As New List(Of String)
        ...
        ''' <returns></returns>
        Friend Shared Function getRecordName(Of T As BaseModel)(cp As CPBaseClass, recordId As Integer) As String
            Try
                If (recordId > 0) Then
                    Dim instanceType As Type = GetType(T)
                    Dim tableName As String = derivedContentTableName(instanceType)
        ...
        ''' <returns></returns>
        Friend Shared Function getRecordName(Of T As BaseModel)(cp As CPBaseClass, ccGuid As String) As String
            Try
                If (Not String.IsNullOrEmpty(ccGuid)) Then
                    Dim instanceType As Type = GetType(T)
                    Dim tableName As String = derivedContentTableName(instanceType)
        ...
        ''' <param name="ccGuid"></param>record
        ''' <returns></returns>
        Friend Shared Function getRecordId(Of T As BaseModel)(cp As CPBaseClass, ccGuid As String) As Integer
            Try
                If (Not String.IsNullOrEmpty(ccGuid)) Then
                    Dim instanceType As Type = GetType(T)
                    Dim tableName As String = derivedContentTableName(instanceType)
        ...
        '
        '====================================================================================================
        Friend Shared Function getCount(Of T As BaseModel)(cp As CPBaseClass, sqlCriteria As String) As Integer
        ...
        ''' <returns></returns>
        Friend Function getUploadPath(Of T As BaseModel)(fieldName As String) As String
            Dim instanceType As Type = GetType(T)
            Dim tableName As String = derivedContentTableName(instanceType)
