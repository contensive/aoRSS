...
Namespace Controllers
    ...
    Public Class applicationController
        ...
        ''' </summary>
        ''' <remarks></remarks>
        Public Sub New(cp As BaseClasses.CPBaseClass, Optional requiresAuthentication As Boolean = True)
            Me.cp = cp
            Dim sql As String = ""
            Dim cs As CPCSBaseClass = cp.CSNew()
            Dim localSystemStatus As String = ""
            If (requiresAuthentication And Not cp.User.IsAuthenticated) Then
                packageErrorList.Add(New packageErrorClass() With {.number = constants.resultErrorEnum.errAuthentication, .description = "Authorization is required."})
                cp.Response.SetStatus(httpErrorEnum.forbidden & " Forbidden")
            End If
        End Sub
        '
        Public Shared Function serializeObject(ByVal CP As BaseClasses.CPBaseClass, ByVal dataObject As Object) As String
            Dim result As String = ""
            Try
